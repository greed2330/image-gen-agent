...
class PromptCompiler:
    ...
    async def compile(self, intent: Intent, route: RouteDecision) -> CompiledPrompt:
        ...
        # Excluded = tags the user explicitly does NOT want → forced to negative, stripped from positive.
        exclude_set = {_normalize_tag(t) for t in intent.exclude_tags if t.strip()}
        ...
        # Classify TIPO output: skip duplicates/excluded, move conflicts to negative, keep the rest
        ...
        for t in expanded:
            ...
            if nt in exclude_set:
                continue  # user excluded it — keep out of positive (already forced to negative)
        ...
        negative = _dedup(quality_neg + conflict_neg + sorted(exclude_set) + censored_found + forced_neg)
